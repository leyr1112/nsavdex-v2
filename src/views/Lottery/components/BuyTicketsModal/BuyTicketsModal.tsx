...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInNsbc)
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInNsbc.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInNsbc, discountDivisor, getTicketCostAfterDiscount])
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInNsbc.times(new BigNumber(ticketsToBuy))) : '0.00'} NSBC`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInNsbc && getFullDisplayBalance(priceTicketInNsbc.times(ticketsToBuy || 0))} NSBC
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
