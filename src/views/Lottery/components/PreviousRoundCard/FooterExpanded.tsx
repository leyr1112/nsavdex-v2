...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInNsbc } = lotteryNodeData
    prizeInBusd = amountCollectedInNsbc.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" NSBC"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInNsbc)}
            decimals={0}
          />
        )}
      </>
